...
async function getDashboardStats(userId, userEmail) {
  try {
    ...
    const [
      ...
      // Total recipes authored by user
      ...
      // Sum of likesCount across all user's recipes
      ...
      // 5 most recent recipes by this user
      db.collection("recipes")
...
function StatCard({ icon: Icon, label, value, color }) {
  ...
  return (
    <div className="flex items-center gap-4 rounded-2xl border border-separator bg-surface p-5">
      ...
      <div>
        <p className="text-2xl font-bold font-display text-surface-foreground">
          {value}
        </p>
...
export default async function DashboardPage() {
  ...
  try {
    ...
    const fullUser = await db
      .collection("users")
      .findOne({ id: user.id }, { projection: { isPremium: 1, role: 1 } });
    await client.close();
  ...
  const { totalRecipes, totalFavorites, totalLikesReceived, recentRecipes } =
    await getDashboardStats(user.id, user.email);
  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      <div className="flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between">
        ...
        {isPremium ? (
          ...
          <NextLink
            ...
            className="inline-flex w-fit items-center gap-2 rounded-full border border-dashed border-warning/60 px-4 py-2 text-sm font-medium text-warning/80 transition-colors hover:bg-warning/10"
          >
            <FaCrown className="size-4" />
            Upgrade to Premium
          </NextLink>
      ...
      {/* Recipe limit notice for free users */}
        ...
        <div className="flex flex-col gap-3 rounded-2xl border border-dashed border-warning/50 bg-warning/5 p-5 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="mt-0.5 text-xs text-muted">
              Upgrade to Premium to post unlimited recipes.
            </p>
          </div>
          <NextLink
            href="/dashboard/premium"
            className="inline-flex shrink-0 items-center gap-2 rounded-xl bg-warning px-4 py-2 text-sm font-semibold text-warning-foreground transition-opacity hover:opacity-90"
          >
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-separator bg-surface">
                  ...
                  <th className="px-4 py-3 text-right font-semibold text-surface-foreground">
                    Likes
                  </th>
                </tr>
              </thead>
